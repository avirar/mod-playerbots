...
void NewRpgStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("go camp status", NextAction::array(0, new NextAction("new rpg go camp", 3.1f), nullptr)));

    triggers.push_back(
        new TriggerNode("wander random status", NextAction::array(0, new NextAction("new rpg wander random", 3.0f), nullptr)));

    triggers.push_back(
        new TriggerNode("wander npc status", NextAction::array(0, new NextAction("new rpg wander npc", 3.1f), nullptr)));

    triggers.push_back(
        new TriggerNode("do quest status", NextAction::array(0, new NextAction("new rpg do quest", 3.1f), nullptr)));

    triggers.push_back(
        new TriggerNode("travel flight status", NextAction::array(0, new NextAction("new rpg travel flight", 3.1f), nullptr)));
}
