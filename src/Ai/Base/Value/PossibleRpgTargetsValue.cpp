...
bool TrainerClassifier::IsValidSecondaryTrainer(Player* bot, Creature* trainer)
{
    ...
    std::vector<Trainer::Spell> const& trainerSpells = trainerData->GetSpells();
    if (trainerSpells.empty()) {
        if (botAI && botAI->HasStrategy("debug targets", BOT_STATE_NON_COMBAT))
        {
    ...
    for (Trainer::Spell const& tSpell : trainerSpells)
    {
        if (trainerData->CanTeachSpell(bot, &tSpell)) {
            hasLearnableSpells = true;
            greenSpellCount++;

            // Check if this learnable spell teaches primary profession skills
            if (TeachesPrimaryProfession(&tSpell, botAI)) {
                ...
                if (botAI && botAI->HasStrategy("debug targets", BOT_STATE_NON_COMBAT))
                {
                    LOG_DEBUG("playerbots", "[TrainerClassifier] {} - Trainer {} teaches primary profession spell {} (skill: {})",
                              bot->GetName(), trainer->GetName(), tSpell.SpellId, tSpell.ReqSkillLine);
                }
...
bool TrainerClassifier::TeachesPrimaryProfession(Trainer::Spell const* tSpell, PlayerbotAI* botAI)
{
    // Check ReqSkillLine category
    if (tSpell->ReqSkillLine > 0) {
        // Riding skill (762) should not be treated as a primary profession
        if (tSpell->ReqSkillLine == 762) { // SKILL_RIDING
            if (botAI && botAI->HasStrategy("debug targets", BOT_STATE_NON_COMBAT))
            {
                ...
                if (category == SKILL_CATEGORY_PROFESSION) {
                    if (botAI && botAI->HasStrategy("debug targets", BOT_STATE_NON_COMBAT))
                    {
                        LOG_DEBUG("playerbots", "[TrainerClassifier] Spell {} teaches skill {} (category {}=PROFESSION) via effect",
                                  tSpell->SpellId, skillId, category);
                    }
                    return true;
                }
            }
