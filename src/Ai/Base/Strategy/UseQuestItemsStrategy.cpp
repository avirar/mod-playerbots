...
void UseQuestItemsStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("far from quest item target", { NextAction("move to quest item target", 6.0f) }));

    // Use quest items when in range
    triggers.push_back(new TriggerNode("quest item usable", { NextAction("quest item use on target", 5.5f) }));

    // Periodic quest item check
    triggers.push_back(new TriggerNode("often", { NextAction("quest item use on target", 5.0f) }));

    // Quest spell actions: same priority band as quest items
    // Use quest spells (e.g., Gift of the Naaru on Draenei Survivors)
    triggers.push_back(new TriggerNode("often", { NextAction("quest spell use on target", 5.0f) }));
}
